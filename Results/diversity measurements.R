...
fig_1 <- (ggplot(shannon, aes(x = Region, y = Shannon))+
         geom_bar(position = position_dodge(), stat = "identity", colour = "black", fill = "darkred") +
         theme_bw()+
...
# changing the order of evidence types on the heatmap so it can be interpreted via patterns more easily 
isra_long <- pivot_longer(data = isra_matrix,
                          cols = -c(Region),
                          names_to = "Evidence type",
                          values_to = "Abundance")
...
#heatmap of regions and evidence types
fig_6 <- ggplot (data = isra_long, mapping = aes(x = Region,
                                                 y = factor (`Evidence type`,
                                                 levels = c("Artisanal surveys",
                                                            "Bottom trawling",
                                                            "Catch and bycatch data",
                                                            "Commercial data",
                                                            "Demersal gillnets and hand net surveys",
